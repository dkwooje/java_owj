...
public class studyA1 {
	public static void main(String[] args) {
		...
		System.out.println("ds1의 바인딩 정보: " + ds2.getLocalAddress()+":"+ ds2.getLocalPort());
		System.out.println("ds1의 바인딩 정보: " + ds3.getLocalAddress()+":"+ ds3.getLocalPort());
		System.out.println("ds1의 바인딩 정보: " + ds4.getLocalAddress()+":"+ ds4.getLocalPort());
		...
		System.out.println(" 원격 주소 정보 : " + ds4.getInetAddress()+ ":"+ ds4.getPort());
		ds4.disconnect();
		System.out.println();
		...
		try {
			...
			ds2.send(dp2);
			ds3.send(dp2);
			//소켓-연결 정보 있음 + 패킷-수신지 정보 있음
			//가능: 두 주소 일치: 소켓-연결 주소 = 패킷-수신지 주소
			ds3.connect(new InetSocketAddress("localhost", 10002));
			ds3.send(dp2);
			ds3.disconnect();
			// 불가능 : 두 주소 불일치 : 소켓-연결(connect) 주소 != 패킷-수신지 주소
			// ds3.connect(new InetScocketAddress("localhost", 10003));
			// ds3.send(dp2); // IllegalArumentException
			// ds3.disconnect(); 
			
			//데이터 수신
			byte[] receivedData = new byte[655058];
			DatagramPacket dp = new DatagramPacket(receivedData, receivedData.length);
			for(int i=0; i<7; i++) {
				ds4.receive(dp);  //dp에 데이터를 담는다.
