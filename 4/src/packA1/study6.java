...
public class study6 {
	public static void main(String[] args)  throws IOException{
		...
		//3. n-byte 단위 읽기(앞에서 length만큼 읽어 bytep[] offset 위치에 넣기)
		InputStream is3 = new FileInputStream(inFile);
		byte[] byteArray2 = new byte[9]; //최소 offset + length
		int offset =3;
		int length =6;
		int count2 = is3.read(byteArray2, offset, length);
		String str = new String(byteArray2, 0, offset + count2, Charset.forName("UTF-8"));
		//String str = new String(byteArray2, offset, count2, Charset.default Charset());
